...
public class DefaultEventQueueImpl<T extends EventMessage> extends AbstractEventQueue<T> {
    ...
    public DefaultEventQueueImpl(Executor executor) {
        super(new AsyncEventDispatchProcessor<>(executor));
        subscribers = new HashSet<>();
    }

    public DefaultEventQueueImpl(Executor executor, int unitOfSubscription) {
        super(new AsyncEventDispatchProcessor<>(executor, unitOfSubscription));
        subscribers = new HashSet<>();
    }
